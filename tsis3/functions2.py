...
num = int(input())
is_greater = check_score_greater(movies[num])
if (is_greater):
    print('True')
else:
    print('False')
...
def sublist_movies_high_score(movies):
    ...
    for i in range(0, len(movies)):
        ...
        if curr_movie['imdb'] > 5.5:
            out_list.append(curr_movie['name'])
    return out_list
...
print('')
print('Average IMDB score of all the movies is: ', end='')
s1=avg_imdb_score(movies)
print(s1)

def return_movie_cat(movies, cat_name):
    out_list = []
    for i in movies:
        curr_cat = i['category']
        if cat_name.lower() == curr_cat.lower():
...
def avg_imdb_acc_to_cat(movies,cat_name):
    cat_movies=return_movie_cat(movies,cat_name)
    avg_score=avg_imdb_score(cat_movies)
    return avg_score

print('')
print(f'Average IMDB of movies in the {cat} category is:')
s2=avg_imdb_acc_to_cat(movies,cat)
print(s2)